...
# URLs ที่จะตรวจสอบ
url_1 = ""
url_2 = "https://hyp-api-beta.mihoyo.com/hyp/hyp-connect/api/getGameBranches?game_ids[]=j7rlly0oYR&launcher_id=TC4836G73s"
    ...
    for name, webhook_url in webhook_urls.items():
        try:
            ...
            for chunk in chunks:
                ...
                if len(chunk) <= 6000:
                    embeds.append({
                        "title": f"{name} - Honkai Nexus Anima CN BETA {source}" if len(embeds) == 0 else None,
                        "description": f"```json\n{chunk}\n```",
                        "color": 0x00ecff,
                        "footer": {
