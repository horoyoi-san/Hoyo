...
def has_changed(api_url, filename):
...
for index, api_url in enumerate(api_urls, start=1):
    try:
        ...
        if not has_changed(api_url, f"last_hash_{index}.txt"):
            print(f"No change for API {index}, skipping webhook")
            continue
        ...
        game_data = next(g for g in resp["data"]["games"] if g["id"] == "4ziysqXOQ8")
        ...
        game_data_list.append((game_name, combined_main))
        ...
        for patch in game_package["main"].get("patches", []):
            ...
            game_data_list.append((f"{game_name} {patch_version} - Hdiff", combined_patch))
        ...
        if pre_major:
            ...
            game_data_list.append((f"{game_name} Pre-Download", combined_pre))
        ...
        for patch in pre.get("patches", []):
            ...
            game_data_list.append((f"{game_name} Pre-Download {patch_version} - Hdiff", combined_pre_patch))

        # ส่ง webhook
        for webhook_url in webhook_urls:
            if webhook_url:
                for title, lines in game_data_list:
                    split_and_send(webhook_url, title, lines, icon_url, bg_url, game_name)
