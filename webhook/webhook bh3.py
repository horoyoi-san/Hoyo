...
CHANNEL_ID = os.getenv("DISCORD_BH3_CHANNEL_ID", os.getenv("DISCORD_CHANNEL_ID", "YOUR_CHANNEL_ID"))
...
def send_discord_message(payload):
    ...
    raise RuntimeError(
        "Please set DISCORD_WEBHOOK_URL or set DISCORD_BOT_TOKEN and DISCORD_BH3_CHANNEL_ID."
    )
...
for block in blocks:
    m = re.search(r"Honkai Impact 3 (\d+)\.(\d+)", block)
...
for major in sorted(groups.keys()):
    ...
    for desc in embeds_to_send:
        payload = {
            "embeds": [
                {
                    "title": f"Honkai Impact 3 Timeline (STC) ({major}.0)",
                    "description": desc,
                    "color": PURPLE,
                    "image": {
                        "url": GIF_URL
