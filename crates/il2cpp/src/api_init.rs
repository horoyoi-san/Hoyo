...
pub fn init_il2cpp(debug: bool) {
    unsafe {
        ...
        for i in 0..assembly_count {
            ...
            crate::MAX_TYPEDEFINDEX.fetch_add(class_count, Ordering::Relaxed);

            if module == "RPG.Network.Proto.dll" {
                crate::RPG_NETWORK_PROTO_START.store(type_def_index, Ordering::Relaxed);
                crate::RPG_NETWORK_PROTO_END.store(type_def_index + class_count, Ordering::Relaxed);
            } else if module == "Assembly-CSharp.dll" {
                crate::ASSEMBLY_CSHARP_START.store(type_def_index, Ordering::Relaxed);
            }
        ...
        log::debug!(
            ...
            method_maps.len(),
            crate::MAX_TYPEDEFINDEX.load(Ordering::Relaxed) - 1
        );
