use std::{borrow::Cow, collections::HashMap, sync::{LazyLock, atomic::Ordering}};
...
pub static AVATAR_HANDLER_CLASS: LazyLock<Cow<'static, str>> = LazyLock::new(|| {
    ...
    let max = il2cpp::MAX_TYPEDEFINDEX.load(Ordering::Relaxed);
    for i in start..max {
    ...
    log::warn!("[Method NT] failed to find AvatarHandlerClass");
    Cow::Borrowed("")
});
