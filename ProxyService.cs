namespace Proxy
{
    ...
    internal class ProxyService
    {
        ...
        private ProxyService(string targetRedirectHost, int targetRedirectPort, ProxyConfig conf)
        {
            _conf = conf;
        ...
        public static async Task<ProxyService> CreateAsync(string targetRedirectHost, int targetRedirectPort, ProxyConfig conf)
        {
            var proxyService = new ProxyService(targetRedirectHost, targetRedirectPort, conf);
            await proxyService._webProxyServer.CertificateManager.EnsureRootCertificateAsync();
            ...
            await proxyService.SetEndPointAsync(endPoint);

            return proxyService;
        }

        private async Task SetEndPointAsync(ExplicitProxyEndPoint explicitEP)
        {
            explicitEP.BeforeTunnelConnectRequest += BeforeTunnelConnectRequest;

            _webProxyServer.AddEndPoint(explicitEP);
            await _webProxyServer.StartAsync();
