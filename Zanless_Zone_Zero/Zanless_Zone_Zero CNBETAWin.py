...
webhook_urls = {
    ...
    "OSBETAWin": 'https://discord.com/api/webhooks/1315273814132785273/KCEkUloeo75HpgwrEVhXDfRzLSuOB7LHf0Nm1zCme0I1s-bl_jkujpcVZC8KSKifEkNU',
    "OSBETAWin-2": 'https://discord.com/api/webhooks/1313874393532989532/0mN1RuiIcN9zDC4HmE4PIOcAPN7B73tgX2TUHMpQH3EkmRTiy5LizlR1PZsnf-J0RSQs',
    ...
    for name, webhook_url in webhook_urls.items():
        try:
            ...
            for chunk in chunks:
                ...
                if len(chunk) <= 6000:
                    embeds.append({
                        "title": f"{name} - Genshin Impact CN BETA {source}" if len(embeds) == 0 else None,
                        "description": f"```json\n{chunk}\n```",
                        "color": 0x00ecff,
                        "footer": {
            ...
            if embeds:
                for embed in embeds:
                    if len(json.dumps(embed)) <= 6000:
                        response = requests.post(webhook_url, json={"username": "Zanless Zone Zero", "embeds": [embed]})
