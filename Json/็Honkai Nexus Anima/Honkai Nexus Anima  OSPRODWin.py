...
# URLs ที่จะตรวจสอบ
url_1 = ""
url_2 = "https://sg-hyp-api.hoyoverse.com/hyp/hyp-connect/api/getGamePackages?game_ids%5B%5D=U5hbdsT9W7&launcher_id=VYTpXlbWo8"
    ...
    for name, webhook_url in webhook_urls.items():
        try:
            ...
            for chunk in chunks:
                ...
                if len(chunk) <= 6000:
                    embeds.append({
                        "title": f"{name} - Honkai Nexus Anima OS PROD {source}" if len(embeds) == 0 else None,
                        "description": f"```json\n{chunk}\n```",
                        "color": 0x00ecff,
                        "footer": {
