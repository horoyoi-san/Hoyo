...
# URLs ที่จะตรวจสอบ
url_1 = ""
url_2 = "https://sg-hyp-api-beta.hoyoverse.com/hyp/hyp-connect/api/getGameBranches?game_ids[]=4qvmDrMwKS&launcher_id=95ODRGH3xC"
    ...
    for name, webhook_url in webhook_urls.items():
        try:
            ...
            for chunk in chunks:
                ...
                if len(chunk) <= 6000:
                    embeds.append({
                        "title": f"{name} - Honkai Nexus Anima OS BETA {source}" if len(embeds) == 0 else None,
                        "description": f"```json\n{chunk}\n```",
                        "color": 0x00ecff,
                        "footer": {
            ...
            if embeds:
                for embed in embeds:
                    if len(json.dumps(embed)) <= 6000:
                        response = requests.post(webhook_url, json={"username": "Honkai Nexus Anima", "embeds": [embed]})
