...
for index, api_url in enumerate(api_urls, start=1):
    try:
        ...
        game_data = [
            ("Zanless Zone Zero PROD", [f"version: {version}"] + main_game),
            ("Zanless Zone Zero PROD audio", main_audio),
        ]

        for patch in game_package["main"].get("patches", []):
            patch_version = patch["version"]
            game, audio = extract_game_audio(patch)
            game_data.append((f"Zanless Zone Zero PROD {patch_version} - Patch", game))
            game_data.append((f"Zanless Zone Zero PROD {patch_version} - audio", audio))
        ...
        if pre_major:
            ...
            game_data.append((f"Zanless Zone Zero Pre-Download", [f"PRE-version: {pre_version}"] + pre_game))
            game_data.append((f"Zanless Zone Zero Pre-Download audio", pre_audio))

        for patch in pre.get("patches", []):
            patch_version = patch["version"]
            game, audio = extract_game_audio(patch)
            game_data.append((f"Zanless Zone Zero Pre-Download {patch_version} - audio", game))
            game_data.append((f"Zanless Zone Zero Pre-Download audio {patch_version} - audio", audio))
